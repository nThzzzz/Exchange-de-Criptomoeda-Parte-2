    ...
    if(escolha >= 0 && escolha < quantidade){
        ...
        if(qtd_compra <=0){
            ...
            if(roundf((pessoas[usuariologado].reais - (compra + taxa)) * 100.00) / 100.00 >= 0){
              ...
              pessoas[usuariologado].reais = roundf((pessoas[usuariologado].reais - (compra + taxa)) * 100.00) / 100,00; // faz a taxação na cotação e adiciona nos reais
...
void vender(pessoa pessoas[],  moeda *moedas, int usuariologado, int quantidade) {
  ...
  for(int i=0; i<quantidade; i++){
    ...
    puts("\nDigite o numero da moeda que deseja vender, começando por 1: ");
    scanf("%d", &escolha);
    escolha--;
    float qtd_venda;
